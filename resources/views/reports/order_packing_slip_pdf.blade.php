...
<body class="{{ $body_classes }}">
  ...
  <div class="row">
    ...
    <table class="no-border drop-shadow" style="border-style:none">
      <tr>
        <td style="width:70%;padding-top:5px;padding-bottom:20px">
        ...
        <td style="width:30%;margin-left:0px;padding-left:0px">
          <table border="0" style="border:0px;border-style:none;">
            <tr>
              ...
              <td style="border:none;text-align:right;position:relative;right:36px">{{ $subtotal }}</td>
            </tr>
            @if ($order->mealPlanDiscount > 0)<tr>
              <td style="border:none"><b>Subscription Discount</b></td>
              <td style="border:none;text-align:right;position:relative;right:36px">{{ $mealPlanDiscount }}</td>
            </tr>@endif
            @if ($order->salesTax > 0)<tr>
              <td style="border:none"><b>Sales Tax</b></td>
              <td style="border:none;text-align:right;position:relative;right:36px">{{ $salesTax }}</td>
            </tr>@endif
            @if ($order->processingFee > 0)<tr>
              <td style="border:none"><b>Processing Fee</b></td>
              <td style="border:none;text-align:right;position:relative;right:36px">{{ $processingFee }}</td>
            </tr>@endif
            @if ($order->deliveryFee > 0)<tr>
              <td style="border:none"><b>Delivery Fee</b></td>
              <td style="border:none;text-align:right;position:relative;right:36px">{{ $deliveryFee }}</td>
            </tr>@endif
            @if ($order->couponReduction > 0)<tr>
              <td style="border:none"><b>Coupon</b></td>
              <td style="border:none;text-align:right;position:relative;right:36px">({{ $couponCode }}) {{ $coupon }}</td>
            </tr>@endif
            @if ($order->balance > 0)<tr>
            <td style="border:none"><b>Total</b></td>
              <td style="border:none;text-align:right;position:relative;right:36px">{{ $amount }}</td>
            </tr><tr>
            <td style="border:none"><b>Paid</b></td>
              <td style="border:none;text-align:right;position:relative;right:36px">${{number_format($order->amount - $order->balance, 2)}}</td>
            </tr>
            @endif
          </table>
        </td>
