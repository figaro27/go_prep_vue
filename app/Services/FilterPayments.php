...
class FilterPayments
{
    ...
    public function getPayments(Request $request)
    {
                ...
                ->orWhere(function ($query2) use (
                    ...
                    $query2
                        ...
                        ->whereHas('meal_orders', function ($subquery) use (
                            ...
                            $subquery->whereNotNull('meal_orders.created_at');
                            if ($byPaymentDate) {
                                $subquery->where(
        ...
        $orders = $removeCashOrders ? $orders->where('cashOrder', 0) : $orders;
        $orders = $removeManualOrders ? $orders->where('manual', 0) : $orders;

        $orders = $couponId ? $orders->where('coupon_id', $couponId) : $orders;
        ...
        foreach ($orders as $order) {
            ...
            $order->amount =
                $order->amount + $order->chargedAmount - $order->transactionFee;
        }
        ...
        if ($this->store->has('orders')) {
            ...
            foreach ($orders as $order) {
                // Adds any additional charges on the order to the total order amount
                // Calculates and subtracts the total transaction fee (Stripe & GoPrep)
                $order->preTransactionFeeAmount =
