        ...
        <div :class="`col-md-12 main-menu-area menu-page`">
          ...
          <div @click="backFromPackagePage">
            <floating-action-button
              class="d-md-none"
              style="background-color:#808080;margin-bottom:65px"
...
export default {
  ...
  methods: {
    ...
    removeOldDeliveryDates() {
      this.bag.forEach(item => {
        if (item.delivery_day) {
          if (
            moment(item.delivery_day.day_friendly).isBefore(
              moment().startOf("day")
            )
          ) {
            this.clearMealFullQuantity(
    ...
    continueToCheckout(overRide = false) {
      alert(this.groupBag.length);
      alert(this.sortedDeliveryDays.length);
